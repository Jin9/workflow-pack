"""P1/P2 acceptance: full 27-stage replay — dual validation, gates, determinism."""
import asyncio
import hashlib
import json
    ...
    orch = Orchestrator(
        ...
        approve_hook=lambda g: (
            g.spec.proceed_when or ("approve" if "approve" in g.spec.verdicts
                                    else g.spec.verdicts[0]),
            APPROVER, "test approval"),
    )
...
def test_full_replay_27_of_27(tmp_path):
    ...
    assert out["ba_brief"]["artifact"] == "S1b-ba-brief/INDEX.json"

    # the pack exports cleanly with a GREEN-ish board
    pack = build_pack(orch)
    assert len(pack["stages"]) == 27
