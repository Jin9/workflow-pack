...
| Key | Source |
|---|---|
| `run` | run header: folder name as `run_id`; `project`/`produced_by`/`schemaVersion` from the S1b INDEX/brief; `audit_index` collected from every section carrying an `audit_id`; `gate_ledger` = the design's L3 sync-named gates (S2, S2.5, S4c, T10, S5, S6, S7). |
