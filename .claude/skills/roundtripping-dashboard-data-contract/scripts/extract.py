...
def extract(html_path, data_uuid):
    ...
    with tempfile.NamedTemporaryFile(
        ...
        tmp_path = tmp.name
    try:
        # The DATA module is EXECUTED, not parsed — only the workspace-owned
        # bundle is trusted input. Bounded: a non-terminating module fails here.
...
def main():
    ...
    ap.add_argument("--force", action="store_true",
                    help="allow overwriting an existing non-default --out path")
    args = ap.parse_args()

    if os.path.realpath(args.out) == os.path.realpath(args.html):
        raise SystemExit("error: --out resolves to the bundle itself: %s" % args.out)
    if (os.path.exists(args.out) and not args.force
            and os.path.realpath(args.out) != os.path.realpath(DEFAULT_OUT)):
        raise SystemExit(
            "error: refusing to overwrite existing %s (non-default path; pass --force)" % args.out)

    try:
        json_text = extract(args.html, args.data_uuid)
    except subprocess.TimeoutExpired:
        raise SystemExit("error: node evaluation of the DATA module did not terminate (120s)")
