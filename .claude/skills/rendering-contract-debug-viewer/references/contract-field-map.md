...
|---|---|---|---|
| `audit_id`, `idempotency_key`, `*_id`, `id` | every contract | UUID / stable id | monospace chip |
...
| `verdict` | plan-review | `PROCEED` → ok · `REVISE` → warn · `BLOCK` → err | status badge |
| `recommendation` | discovery | `proceed` → ok · `needs-work` → warn · `do-not-build` → err | status badge |
| `status`, `state`, `spec_status` | most | `ready-for-tl`/`ready-for-implementation`/`accepted`/`resolved`/`pass` → ok · `draft`/`proposed`/`pending` → warn · `blocked`/`fail`/`rejected`/`deprecated` → err | status badge |
