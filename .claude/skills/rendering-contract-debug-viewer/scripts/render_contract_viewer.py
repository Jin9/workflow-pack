...
the agent's authored shell (CSS+JS+markup) is offline-checked before the JSON data is baked
in, and data values render as escaped text (never as a resource URL or markup).
...
FORBIDDEN = ("http://", "https://", "src=", "@import")  # checked on the authored shell only
...
const STATUS_KEYS=new Set(['output_type','status','state','verdict','recommendation','spec_status','citation_status','legal_status','coverage_score','parsing_mode']);
const OK=new Set(['proceed','ready-for-tl','ready-for-implementation','ready-for-audit','resolved','pass','accepted','design','ux_pack','brief','done','present','complete']);
const WARN=new Set(['needs-work','draft','revise','partial','partial_design','partial_ux_pack','pending','warn','proposed','reviewed','blocked_partial_brief']);
const ERRV=new Set(['do-not-build','block','blocked','fail','hardfail','blocked_design','blocked_ux_pack','failure_shape','deprecated','superseded','rejected']);
...
function summaryChips(d){
  ...
  const aid=d.audit_id||(d.processing_metadata&&d.processing_metadata.audit_id);
...
try{const s=localStorage.getItem('rcdv-theme');if(s)setTheme(s);}catch(e){}
build();
...
document.getElementById('srcBtn').addEventListener('click',()=>{
  ...
  else{srcEl.setAttribute('hidden','');tree.removeAttribute('hidden');b.textContent='View source';}
});
"""
...
<header class="top">
  ...
  <button id="srcBtn">View source</button>
  <button id="expandBtn">Expand all</button>
  <button id="collapseBtn">Collapse all</button>
  <button id="themeBtn">Toggle theme</button>
</header>
<p class="note">Offline debug view of a pipeline JSON contract, in the squad-delivery dashboard theme. Values are rendered as escaped text. Use "View source" for the pretty-printed JSON.</p>
...
def check_offline(shell_text):
    """Fail closed if the authored shell references any external resource."""
    low = shell_text.lower()
    hits = [tok for tok in FORBIDDEN if tok in low]
    if hits:
        raise ValueError("authored shell contains forbidden external reference token(s): %s" % ", ".join(hits))


def render(data, title, src_label, init_theme):
    # Author-controlled shell (placeholders for data still in place) must be offline-clean.
    shell = SHELL.replace("__CSS__", CSS).replace("__JS__", JS)
    check_offline(shell.replace("__DATA__", "").replace("__TITLE__", "").replace("__SRC__", ""))
