...
- [2026-07-28 — The BA brief gets a gate: four supervision gaps closed](history/2026-07-28-ba-brief-gate.md) — supervision review of the v2 leg found its controls structural where implemented and PROSE where not: the elaborated brief (65 AC, the edge-case ledgers, the banking-grade rows) had no gate and `ready-for-tl` was enforced nowhere, so a `needs-work` brief shipped to tl-design unsigned; the amigos' binding `conditions` were declared input the engine never populated; the documented `blocks_elaboration` re-check could not fire; a blocking gate that was not sync-named took a one-character approver; and a missing gates.yaml disarmed every gate past the BA leg. All five closed by reusing existing mechanism (no new gate machinery), proven adversarially with a counterfactual rather than a green test
- [2026-07-28 — BA leg v2: breakdown + three-amigos gate + elaboration (27→28 stages)](history/2026-07-28-ba-leg-v2.md) — eliciting-banking-brief (16,902 lines, 13 jobs) split into breaking-down-ba-scope + elaborating-user-stories with a QUORUM gate between (distinct named BA/dev/QA signatures, non-agreed verdicts loop back with the reviewers' findings); epics regrouped by business dependency (4→2 on ShopPilot); RULES/DOMAIN/FLOW catalogues added; AC 29→65 while the BA artifact shrank 21%; seven pre-existing defects fixed en route (dangling replay hydration, always-empty smoke subset, vacuous test assertion, drawio loop-back target, 20 dashboard pin drifts)
