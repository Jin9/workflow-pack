...
`P2` routes to `loop_back` (target = `backend-implement`).
...
| Unverified claim check (and no `P1`) | `loop_back` | `backend-implement` |
| Any `P2` finding (and no `P1` / no unverified claims) | `loop_back` | `backend-implement` |
| `uncertainty_flag` of kind `design_ambiguity` raised in step 2 | `human-queue` | null (this stage CANNOT loop to tl-design — a named human re-routes) |
| Only `P3` findings (and no `P1` / no `P2` / no unverified claims / no design ambiguity) | `approve` | null |
| No findings at all | `approve` | null |

Notes:

- `design_ambiguity` always wins routing — fixing it at `backend-implement` is
  treating the symptom, but the pipeline has no review→design edge, so it
  parks at `human-queue` for a named human to re-route.
