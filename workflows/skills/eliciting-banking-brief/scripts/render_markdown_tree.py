...
def render_failure(data: Dict[str, Any]) -> str:
    ...
    # Declared contract fields only (schemas/output.json failure_state) — the old
    # recommended_action / unblock_actions aliases were never schema-declared.
    rec = failure.get("recommended_questions") or failure.get("suggested_next_action")
    if rec:
...
def render_tree(input_path: Path, output_dir: Path, schema_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    # Destructive-write guard: only ever replace a directory this renderer owns.
    # The target must resolve inside the caller-authorized output_dir, must match
    # the canonical output-{idem8} name, and must not be (or sit behind) a symlink
    # — refuse anything else instead of rmtree'ing an arbitrary path.
    target = target.resolve()
    base = output_dir.resolve()
    if target.name != f"output-{idem8}" or not (target == base or str(target).startswith(str(base) + os.sep)):
        return {"ok": False,
                "errors": [f"refusing to write outside the authorized output dir: {target}"],
                "warnings": [], "files_written": []}
    if target.exists() and target.is_symlink():
        return {"ok": False,
                "errors": [f"refusing to replace a symlink target: {target}"],
                "warnings": [], "files_written": []}

    # Atomic replace: render into a sibling temp dir, then swap. A crash mid-render
    # leaves the previous pack intact (never a half-deleted tree); the displaced
    # previous pack is removed only after the swap succeeds.
    staging = target.parent / f".{target.name}.staging"
    if staging.exists():
        shutil.rmtree(staging)
    staging.mkdir(parents=True)
    prev = target.parent / f".{target.name}.previous"

    files_written: List[Path] = []
    target, final_target = staging, target
    ...
    # Promote the fully-rendered staging dir; the previous pack is displaced and
    # removed only AFTER the swap succeeds (never a half-deleted tree).
    if prev.exists():
        shutil.rmtree(prev)
    if final_target.exists():
        final_target.rename(prev)
    staging.rename(final_target)
    if prev.exists():
        shutil.rmtree(prev)
    files_written = [final_target / p.relative_to(staging) for p in files_written]
    return _finalize(final_target, files_written, schema_errors, cap_warnings)
