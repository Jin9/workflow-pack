...
version: 2.0.1
description: Convert raw BA input (Jira, Slack, meeting notes, email, mixed prose) into a structured epic-plus-stories brief with banking-grade fields force-evaluated, ambiguities surfaced as open questions, and governance gaps escalated as P1 blockers. This IS the live pipeline's ba-research stage (S1 composite, after researching-ba-problem-space). Use when a user submits a Jira ticket, Slack export, meeting transcript, or mixed-source brief for BA elicitation. Use when raw input describes a banking workflow (KYC, EDD, AML, wires, sanctions) needing INVEST stories with Gherkin AC for TL handoff. Do NOT use for TL-stage design (use designing-tech-lead-handoff), code generation from a finished spec (use implement-backend-feature or implement-frontend-feature), domain-glossary lookups with no work request, or inputs carrying the training ground-truth annotation block. PII-bearing input is redacted at preflight, never refused and never echoed.
stage_type: analyze
...
Required: `raw_request` (the raw BA input — Jira ticket, Slack thread, meeting
notes, email, or mixed/doc prose; per-type detection markers in
`references/procedure-detail.md` Step 1), `requester` (named human, provenance),
`normalized_request` (intake's PII-redacted normalized-request STRING — the scope sheet itself is
NOT passed downstream), `idempotency_key` (engine-injected).
Very short input is **not rejected** — it yields `output_type:
needs_clarification` (EC-01/FM-01). **PII redaction is authoritative at
preflight**: redact actual PII values (NRIC, account numbers, credit cards) as
`<PII:REDACTED:CLASS=...>` BEFORE analysis, never reproduce a value, and emit a
P1-blocked brief; `pii_echo_blocked` fires only when a clean output cannot be
guaranteed. **Optional engine-nested `discovery` object** (S1 composite chain,
from `researching-ba-problem-space` after its human gate — delivered under the
`discovery` key by the engine's NESTED_HANDOFF): **advisory only** — it may
*raise* a tier floor, *seed* the absent-stakeholder enumeration via
`discovery.handoff_to_intake.stakeholder_hints`, and *pre-populate*
`regulatory_dependencies` rows for verification; it can NEVER suppress a
detector, lower a tier, satisfy a citation, or replace `raw_request`. An absent
`discovery` is **byte-identical** to pre-handoff behaviour. Idempotency extends
to `(raw_request, discovery)`. Optional engine-injected: `upstream_artifacts`,
`loop_back_feedback`. Threading into Steps 1 / 3 / 5–6 / 11 / 12:
`references/procedure-detail.md` (discovery paths are
`discovery.handoff_to_intake.tier_signal` / `.stakeholder_hints`).

**Example (validates against schemas/input.json):**
```json
{
  "raw_request": "Jira DGL-1234: customers need passwordless login for the mobile app...",
...
**Key invariants enforced by schema:** `frontmatter.status: "ready-for-tl"` ⟹ no P1 OQs AND no `blocks_tl_handoff: true` gaps. Every `banking_grade_concerns` row has `justification.minLength ≥ 10`. `scope_kind: multi-epic` ⇔ `epics[]` + `initiative` present, `epic` absent (both conditionals guard on `scope_kind` being PRESENT — failure shapes without it validate). A `brief` carries non-empty `stories[]` and `blocks_tl_handoff: false`; a `partial_brief` carries ≥1 complete story + `failure_state` naming the incomplete epics (zero complete stories ⟹ `needs_clarification` instead). Failure routing uses the DECLARED `failure_state` fields only (`failure_code`, `recommended_questions`, `blocking_items`, `suggested_next_action`) — the old `reason_code`/`next_action`/`unblock_actions` aliases are retired.

**Provenance id.** Required top-level `audit_id` — producer-stamped, deterministic:
UUIDv5(HOUSE_NS, "ba-research:{idempotency_key}") with HOUSE_NS =
uuid5(NAMESPACE_URL, "https://squad-delivery/audit"). The canonical brief and the
INDEX manifest carry the **same** value; derivation is independent of the optional
`discovery` input (preserving the discovery-absent byte-identity) and distinct
from both `discovery.audit_id` (recorded as
`frontmatter.upstream_refs.discovery_audit_id`) and the engine's per-attempt
execution audit id.

**The pipeline artifact is the manifest, not the brief.** The stage emits the
3-level ref-chain — `INDEX.json` → `EPIC-<DOMAIN>.json` → `STORY-…json` — whose
shapes are target-owned here: `schemas/manifest.json`, `schemas/epic-sidecar.json`,
`schemas/story-sidecar.json`. The manifest's `state` (`ready-for-tl` |
`needs-work` | `blocked`) is the TL readiness signal (a stage required_field).
Consumers never see the manifest raw: the engine hydrates `epics[].file` /
`story_files[].file` into inline sidecar objects and ADDS a `stories` key
alongside the retained `story_files` refs (engine/mapping.py
`_hydrate_ba_research`) — downstream input contracts reference the sidecar
schemas, never inline byte-copies.

**Example (validates against schemas/output.json — minimal failure shape):**
```json
{
  "output_type": "needs_clarification",
  "blocks_tl_handoff": true,
  "frontmatter": {
    "id": "EPIC-UNKNOWN", "workload_tier": "T2", "created_at": "2026-07-12",
    "created_by": "eliciting-banking-brief 2.0.0", "source_ref": "jira:DGL-1234",
    "source_type": "jira", "idempotency_key": "3f6c0b2e-7a41-4d9b-9c2a-8e5b1f0a4d22",
    "ba_confidence": "refused", "status": "blocked",
    "upstream_refs": { "source_artifacts": ["jira:DGL-1234"] },
    "downstream_will_be_consumed_by": { "stage": "tl-design", "role": "Tech Lead" }
  },
  "processing_metadata": { "parsing_mode": "generic_prose_fallback" },
  "failure_state": { "mode": "FM-01", "reason": "Input below quality composite threshold.",
    "failure_code": "empty_or_minimal_input", "suggested_next_action": "prompt_requester" },
  "audit_id": "3f6c0b2e-7a41-4d9b-9c2a-8e5b1f0a4d22"
}
```
