...
missing requirement in the `ba_brief` — never on style or taste.
...
Set `confidence` lower for thin evidence, missing `ba_brief`, or a
non-heterogeneous reviewer. The skill stops at the verdict; the workflow engine
routes REVISE back to the planner and BLOCK to a human.
