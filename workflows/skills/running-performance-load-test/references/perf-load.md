...
Use the `performance_test_context.budget` thresholds exactly as supplied by the approved workflow input (SLI/SLO design lives with `designing-tech-lead-handoff`). This
...
scales, or changes config. Budget/SLO authoring belongs to the design leg (`designing-tech-lead-handoff`);
