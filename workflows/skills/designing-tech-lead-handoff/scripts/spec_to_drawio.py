...
def ecell(eid, value, style, parent, source, target, exit=None, entry=None, points=None,
    ...
    geo = '<mxGeometry relative="1" as="geometry">%s</mxGeometry>' % pts
    if label_x is not None:
        # shift the edge label along the path (-1 source .. 0 mid .. 1 target)
        geo = '<mxGeometry x="%g" relative="1" as="geometry">%s</mxGeometry>' % (label_x, pts)
    return ('<mxCell id="%s" value="%s" style="%s" parent="%s" edge="1" source="%s" target="%s">'
            '%s</mxCell>') % (eid, esc(value), st, parent, source, target, geo)
...
def build_l2(s):
    ...
    y = L2_BODY_Y
    svc_pitch = L2_SVC_H + 60  # stacked-service pitch: icon + clearance for its bottom label
    for c in s["contexts"]:
        dbs = ds_by_ctx.get(c["id"], [])
        svcs = [v for v in ctx_services if v.get("context") == c["id"]]
        svc_block = L2_SVC_H + (max(1, len(svcs)) - 1) * svc_pitch
        band_h = max(svc_block + 24, len(dbs) * L2_DB_DY + 14)
        svc_y = y + band_h // 2 - svc_block // 2
        for k, v in enumerate(svcs):
            f, st = status_color(v)
            node(v["id"], v.get("label", v["id"]), pod(f, st),
                 L2_SVC_X, svc_y + k * svc_pitch, L2_SVC_W, L2_SVC_H, "service", y)
    ...
    def route(a, b, ka, kb, off):
        ...
        if ka == "service" and kb == "service":
            # first two channels keep their legacy slots; further ones fold left so the
            # channel never enters the service column (L2_SVC_X)
            dx = 0 if off == 0 else (14 if off == 1 else -(off - 1) * 14)
            cx = L2_CH_SS + dx
    ...
    n_ss = sum(1 for t in s["topology"]
               if t.get("from") in boxes and t.get("to") in boxes
               and category(kind[t["from"]], kind[t["to"]]) == "ss")
    seen, chan = {}, {}
    for t in s["topology"]:
        ...
        st, lx = conn_style(t.get("kind")), None
        if cat == "ss" and n_ss > 2:
            # dense same-column bundle: spread labels along their edges + opaque label
            # background so channel lines can't strike through the text
            lx = round(-0.5 + off * (1.0 / (n_ss - 1)), 3)
