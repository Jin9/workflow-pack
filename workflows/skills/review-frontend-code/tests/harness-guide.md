...
| Structural-diff against `expected.json` | `verdict`, `loop_back_target_stage`, finding *categories* + *severities* (not full evidence text), `claim_checks` statuses (by `claim_ref`), `a11y_verdict` enum values, `security_verdict` booleans | Fail with diff |
...
| `UnverifiedClaimsAssertion` | an `unverified` claim check AND no `P1` → verdict `loop_back` to `frontend-implement` (each unverified check carries `finding_ref`). | severity-guide.md claims rule |
...
| `PiiCoverageAssertion` | Every field in input's `implement_stage_output.security_review.pii_fields_handled[]` has a `claim_checks[]` entry (any status) — none dropped. | C4 + F9 |
| `AuditEventCoverageAssertion` | Every entry in `implement_stage_output.audit_events_emitted[]` has a `claim_checks[]` entry. | C3 + F12 |
...
2. Common cases worth adding:
   ...
   - `006-unverified-audit-event.input.json` — Generate claims event `loan.submitted` but code calls `track({event_type: 'loan.created', ...})`. Expected: `loop_back` to `frontend-implement`, an `unverified` claim check with `finding_ref`.
