...
| Unverified claim check (and no `P1`) | `loop_back` | `frontend-implement` |
| Any `P2` finding (and no `P1` / no unverified claims) | `loop_back` | `frontend-implement` |
| `uncertainty_flag` of kind `design_ambiguity` raised in step 2 | `human-queue` | null (no review→design edge exists) |
| `uncertainty_flag` of kind `bundle_overrun` from Generate, no other findings | `human-queue` | null (budget is a design decision — a named human re-routes) |
| `uncertainty_flag` of kind `token_gap` from Generate, no other findings | `human-queue` | null (token system is a design decision — a named human re-routes) |
| Only `P3` findings (no `P1` / no `P2` / no unverified claims / no design-routing flag) | `approve` | null |
| No findings at all | `approve` | null |

Notes:

- `design_ambiguity` / `bundle_overrun` / `token_gap` are design decisions —
  treating the symptom at `frontend-implement` won't help, and the pipeline
  has no review→design edge, so they park at `human-queue`.
