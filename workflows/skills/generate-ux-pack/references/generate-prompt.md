...
`{idem8}` is the first 8 chars of the injected workflow `idempotency_key` — never a freshly generated UUID; a retry with the same key validates/updates the SAME pack.
...
- idempotency_key: the injected workflow idempotency_key (NEVER generate a fresh UUID; retries reuse the same key and pack)
