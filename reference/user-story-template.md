...
<!-- EXAMPLE: Meridian Flexi Loan — new loan -->
...
<!-- EXAMPLE: "Income -> Interest-rate tiers": income source (SA/SE) x Employer payroll (Yes/No) x monthly income -> annual rate.
...
<!-- EXAMPLE: rates follow whatever the risk engine returns; the driving inputs are income, income source, employer payroll, and the effective dates. -->
...
<!-- EXAMPLE: 1. Renaming the "interest rate" label on the mobile-app input screen (owned by the Mobile App team). -->
...
<!-- EXAMPLE: 1. Loan Decision — LOS to DLP (Before/After JSON): the After payload adds a tierList[] array
...
<!-- EXAMPLE: - PROJ-XXXX  `<short subtask title>` -->
