...
Blueprint repository for scaffolding backend platform microservices in Go. Clone this repo, drop your own domains under `app/`, register them in `router/router.go` and `router/subscriber.go`, and you have a production-ready service with HTTP routing, Kafka event handling, structured logging, and graceful shutdown — all wired up.
...
Infrastructure and utility packages are imported from the shared **common** module at `gitlab.com/example-org/platform/backend/common`:
...
- Go 1.26+
- Access to the private module host `gitlab.com/example-org/platform/backend`
- A valid `.env` file based on `.env.template`

Set `GOPRIVATE` for private modules:

```bash
go env -w GOPRIVATE=gitlab.com/example-org/platform/backend
```
...
```go
import "gitlab.com/example-org/platform/backend/common/logger"
...
```go
import "gitlab.com/example-org/platform/backend/common/serror"
...
```go
import "gitlab.com/example-org/platform/backend/common/wrapper"
import "gitlab.com/example-org/platform/backend/common/app"
...
```go
import "gitlab.com/example-org/platform/backend/common/config"
import myconfig "gitlab.com/example-org/platform/backend/project-skeleton/config"
...
```go
import "gitlab.com/example-org/platform/backend/common/kafka"
...
This repository serves as a **foundational skeleton** for new backend microservices. It ships with no example domains — only the scaffolding (bootstrap, config, deps, router/subscriber hooks, build pipeline) needed to start coding.
